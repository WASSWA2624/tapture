# 116 — Cloud upload: a destination the user chooses, never a sync channel

**Phase** 21 · Cloud upload  |  **Depends on** [005](../01-orchestration/005-dependency-allowlist.md), [024](../02-foundation/024-hashing-service.md), [027](../02-foundation/027-secure-storage-service.md), [038](../03-design-system/038-app-card.md), [041](../03-design-system/041-app-dialog-service.md), [060](../04-data-layer/060-exports-table.md), [062](../04-data-layer/062-repository-interfaces.md), [065](../05-file-storage/065-storage-root.md)  |  **Standard** [STANDARD.md](../STANDARD.md)
...
1. Deliver the interface, the table and the repository first; every backend and the runner are written against them.
   ...
   `SecretKey` entry ([027](../02-foundation/027-secure-storage-service.md)). The repository takes its shape from the
   interfaces of [062](../04-data-layer/062-repository-interfaces.md). `send` reports progress, accepts a starting
...
2. Build the destinations screen over that repository. Each row is the `AppListTile` of
   [038](../03-design-system/038-app-card.md), showing kind, label, folder and the outcome of the last connection
...
3. Write the three backends that need no OAuth. S3 fields are access key, secret, region, bucket, optional prefix and
   ...
   [065](../05-file-storage/065-storage-root.md), and writes through a temporary name then renames, so an interruption
...
5. Put the confirmation sheet in place before any send path exists to bypass it. Show the file name, the byte size
   ...
   [041](../03-design-system/041-app-dialog-service.md) so a caller cannot bypass it by building its own dialog.
6. Finish with the runner and its history. Run the transfer through `runIsolate`
   ([024](../02-foundation/024-hashing-service.md)), streaming byte progress to the interface and honouring the
   ...
   history held by the exports table of [060](../04-data-layer/060-exports-table.md), so a process killed mid-transfer
...
- A signing, WebDAV, picker, OAuth or provider package goes through the dependency allowlist of
  [005](../01-orchestration/005-dependency-allowlist.md) before it is added. The OAuth redirect scheme is registered
