...
1. Build the entry screen first, because everything else here hangs off it. One screen takes the chosen file, passes it
   through the validation gate of [005](../05-file-storage/005-file-storage.md), and detects its kind from that
   ...
   [009](../09-templates/009-templates.md), and a spreadsheet of rows to the purpose step below. An
...
2. Ask the purpose for spreadsheets only; a bundle and a template have no ambiguity to resolve. Records to hold go on
   ...
   [015](../15-data-quality/015-data-quality.md) instead of creating records, so the verification exercise it belongs
...
3. Drive the mapping onto an existing template. Preselect each mapping by header name against the target template's
   ...
   [009](../09-templates/009-templates.md) marks as identity is mapped, and say which one is missing. Show the
   first rows as they would be interpreted, so a wrong mapping is visible before the import runs. Reuse the workbook
   reader, header detection and type inference of [009](../09-templates/009-templates.md) unchanged.
4. Turn the mapped rows into records. Validate per row through the validation engine of
   [015](../15-data-quality/015-data-quality.md) and collect failures with their row number and reason rather than
   ...
   [004](../04-data-layer/004-local-database.md), writing each record of [014](../14-records/014-records.md) with source
...
5. Finish with the summary. Group rows by outcome — created, updated, skipped and failed — each identified by its
   spreadsheet row number, assembled from [003](../03-design-system/003-design-system.md). Export the skipped and failed
...
- Reuse the workbook reader, header detection and type inference of
  [009](../09-templates/009-templates.md) and the mapping interface of
  [009](../09-templates/009-templates.md) unchanged; a second copy of a reader, a header detector or an
  inference rule is a defect, not a shortcut (FE-CONS-02, FE-STR-09).
- Import reuses the duplicate detection of [015](../15-data-quality/015-data-quality.md) and defines no similarity
...
- Rows are validated through the engine of [015](../15-data-quality/015-data-quality.md), never through a check written
  here (FE-CONS-01).
...
- The duplicate choice is asked through the shared dialog service of
  [003](../03-design-system/003-design-system.md), never a dialog of this phase's own (FE-CONS-05).
...
- Importing a reference dataset or creating a template from a workbook. This phase routes both; the dataset importers
  belong to 105 · Reference data and the template flow to [009](../09-templates/009-templates.md).
