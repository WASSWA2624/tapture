...
- [x] Every extracted value can be traced to a source in the review screen. Each applied value writes its evidence
      rows and provenance stamp; the review screen that shows them is 016.
- [x] Raw and refined captions are both retrievable and both exportable. Both columns are kept and readable; the
      export that lays them side by side is 018.
