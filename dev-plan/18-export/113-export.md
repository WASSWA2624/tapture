# 113 — Export: XLSX, CSV, JSON, PDF and ZIP, all produced on device

**Phase** 18 · Export  |  **Depends on** [005](../01-orchestration/005-dependency-allowlist.md), [024](../02-foundation/024-hashing-service.md), [060](../04-data-layer/060-exports-table.md), [066](../05-file-storage/066-project-folder-service.md), [089](../09-templates/089-field-type-registry.md), [108](../13-processing/108-processing.md), [109](../14-records/109-records.md), [110](../15-data-quality/110-data-quality.md), [111](../16-review/111-review.md), [112](../17-meetings/112-meetings.md)  |  **Standard** [STANDARD.md](../STANDARD.md)
...
1. Build `export_request.dart` and `export_validation.dart` first; every writer in this phase takes a request and
   ...
   it unchanged. Read validation results from the engine of [110](../15-data-quality/110-data-quality.md) — its
...
2. Build the scope and options sections. Both are plain sections with no screen of their own. Offer five scopes:
   ...
   [109](../14-records/109-records.md). Recompute the record count whenever the scope changes, off the build method.
...
3. Build `value_formatter.dart`, the one formatter every writer calls. Handle dates, numbers with units, choices with
   ...
   units from [108](../13-processing/108-processing.md); do not parse date strings here.
4. Build `photo_naming.dart` and `photo_rename.dart`. Support the specification's token set, including each context
   ...
   [066](../05-file-storage/066-project-folder-service.md) and de-duplicate against names already taken. Rename on the
...
5. Build `xlsx_writer.dart`, `xlsx_refined_columns.dart` and `xlsx_multi_sheet.dart`. Build a `SheetPlan` per template
   ...
   [111](../16-review/111-review.md). Append rows after the last used row of the sheet, writing native cell types
   through `ExportValueFormatter.typed`. Run the whole write inside the isolate runner of
   [024](../02-foundation/024-hashing-service.md), streaming progress per record batch.
6. Build `xlsx_template_copy.dart` and `xlsx_row_targeting.dart`, so an export against an imported workbook leaves the
   ...
   [102](../09-templates/102-xlsx-mapping-screen.md); append only when the template has no predefined rows. Leave a
...
8. Build `csv_writer.dart`, `json_writer.dart` and `data_dictionary.dart`. CSV writes one file per template in UTF-8
   ...
   [089](../09-templates/089-field-type-registry.md).
...
10. Build `pdf/record_report.dart` and `pdf/inspection_report.dart`, the two record-level reports of A52. The record
    ...
    of [103](../09-templates/103-predefined-rows-import.md) (A15), so it reads in the order the inspector worked
...
12. Build `pdf/minutes_report.dart`, the fifth report of A52: attendance list, agenda items with their decisions and
    ...
    [112](../17-meetings/112-meetings.md) and set them side by side, each labelled as which it is; never present
...
15. Build `export_history_screen.dart`, `export_versioning.dart` and `export_share_action.dart`. Store per export, in
    the table of [060](../04-data-layer/060-exports-table.md): timestamp, operator, formats, the resolved request and
...
- The spreadsheet, PDF, archive and share packages are the ones allowed by
  [005](../01-orchestration/005-dependency-allowlist.md); no second XLSX, PDF or archive dependency is added, and the
