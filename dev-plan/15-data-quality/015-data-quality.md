...
1. Build the engine first; everything else in the phase blocks or allows through it. Every issue carries a field key, a
   ...
   field type registry of [009](../09-templates/009-templates.md) rather than a switch at each call site.
   ...
   rules also cover the identity fields of [009](../09-templates/009-templates.md) being present, and evidence
   ...
   [009](../09-templates/009-templates.md) does nothing until this evaluator exists, so conditional requiredness
...
2. One display widget renders those issues two ways: inline beneath the field a `ValidationIssue` names, and as a
   ...
   [003](../03-design-system/003-design-system.md).
3. Hash the identity values: normalise case, whitespace and punctuation through the normaliser the field type registry
   declares, hash with the service of [002](../02-foundation/002-foundation-services.md), store the result on the record of
   [004](../04-data-layer/004-local-database.md) for fast lookup, and recompute it whenever an identity value is edited.
   Score the five signals — equal identity hash, identical photo hash, near-identical photo hash from the perceptual
   hashing of [013](../13-processing/013-processing.md), the same predefined row of
   [009](../09-templates/009-templates.md) in the same context, and the same name in the same context
...
4. The save-time prompt offers the four outcomes and shows the differing values with them; dismissing it leaves both
   ...
   table of [004](../04-data-layer/004-local-database.md) as related, so each record of
   [014](../14-records/014-records.md) carries a badge linking to its counterpart.
...
6. Conflict detection compares the OCR, caption, reference-data and barcode candidates for a field after
   ...
   [013](../13-processing/013-processing.md). The resolution row shows each candidate beside its source label and its
   evidence, and accepts a typed value matching none of them through the field editor of
   [009](../09-templates/009-templates.md). Storing the choice records the value, the source it came from and
   the reason given, then marks the conflict resolved.
7. Verification mode is a toggle in project settings ([008](../08-projects/008-projects.md)) and in the session;
   with it on, capture opens the identifier lookup of [012](../12-capture/012-capture.md) before the form. Prefill
   fills every mapped field from the matched reference row through the lookup binding of
   [010](../10-reference-data/010-reference-data.md) and marks the record on-register, keeping the register values in
...
8. On approval, compare as-recorded with as-found field by field and write the result to the variances table of
   [004](../04-data-layer/004-local-database.md), classifying every mapped field `match`, `changed` or `missing` over
   ...
   produced no record, and checklist rows of [009](../09-templates/009-templates.md) never captured.
...
10. The quality summary answers what still blocks a clean export: how many records are invalid, how many duplicate
    ...
    111 · Review, or a filtered record list from [014](../14-records/014-records.md).
...
- The evaluator takes no template repository, database or Flutter dependency, so the field editor of
  [009](../09-templates/009-templates.md), and later export and import, call it directly (FE-CONS-01).
...
- Every resolution writes an audit row in [004](../04-data-layer/004-local-database.md) naming the choice, both
  record ids and the person (FE-SEC-09).
- The prompt asks one question through the shared dialog service of
  [003](../03-design-system/003-design-system.md), never its own dialog (FE-CONS-05, FE-SIMP-07).
