  ...
  group('app email field', () {
    for (final ({String name, ThemeData theme}) mode in _modes) {
      testWidgets('gallery in ${mode.name}', (WidgetTester tester) async {
        ...
        await expectLater(
          find.byType(AppPage),
          matchesGoldenFile('goldens/app_email_field_${mode.name}.png'),
        );
      });
    }
  });
...
Future<void> _pumpGallery(WidgetTester tester, ThemeData theme) async {
  ...
  await tester.pumpWidget(
    MaterialApp(
      ...
      theme: theme,
      home: const AppPage(title: 'Email fields', body: _GalleryBody()),
    ),
  );
}
...
class _GalleryBodyState extends State<_GalleryBody> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: <Widget>[
        AppEmailField(label: 'Empty', controller: _empty),
        const SizedBox(height: Space.x4),
        AppEmailField(label: 'Filled', controller: _filled),
        const SizedBox(height: Space.x4),
        AppEmailField(
          label: 'Error',
          controller: _error,
        ...
        const SizedBox(height: Space.x4),
        AppEmailField(label: 'Disabled', controller: _disabled, enabled: false),
      ],
    );
  }
}
