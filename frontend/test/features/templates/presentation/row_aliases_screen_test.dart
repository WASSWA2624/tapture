...
void main() {
  testWidgets('an empty template renders through AsyncValueView', (
    ...
    expect(find.text(Copy.rowAliasesEmptyHeadline), findsOneWidget);
  });
  ...
  testWidgets('a failed load renders through AsyncValueView', (
        ...
        templateListProvider.overrideWith(
          (Ref _) => Stream<List<TemplateDef>>.error(
            const StorageFailure(
              message: 'The row aliases could not be read.',
    ...
    expect(find.text('The row aliases could not be read.'), findsOneWidget);
  });
}
  ...
  await tester.pumpWidget(
    ProviderScope(
      ...
      child: MaterialApp(
        ...
        home: RowAliasesScreen(templateId: templateId),
      ),
    ),
  );
}
