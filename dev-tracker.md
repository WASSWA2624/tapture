...
**99 of 281 tasks complete (35.2%)** · last updated 2026-09-20
...
| 099 — Template versioning and record migration | 2026-09-20 | `TemplateVersioning` records the consecutive field diff on any structural bump and stores the previous shape so a record can still render and export under the version it was captured at. The migration screen lists added, removed and retyped fields with record counts and requires `showAppConfirm` before `apply` persists the whole list as one result — a failed write leaves every record behind. `TemplateRepository.save` remains the one version bumper. Guarded by a keep-version-and-diff unit test, a failed-apply unit test, and empty/failure/confirm widget tests. |
