...
5. **Stay inside the feedback.** Do not invent features, redesigns or refactors that no entry asks for. A
   root cause you uncover counts as inside the feedback; an unrelated improvement does not.
...
4. **Merge.** Put entries in one prompt when one change resolves them all: the same root cause, the
...
5. **Split.** Break an entry, or a merged group, into several prompts when it:
...
6. **Order** the prompts by priority first:
...
7. **Write** the prompts using the template in section 5, and `INDEX.md` as described in section 7.
8. **Self-check** against section 8 and fix anything that fails.
...
## Goal
One to three sentences: the observable outcome, on which screens, form factors and themes.
...
## Acceptance criteria
- [ ] Testable statements, one behaviour each, covering every form factor or theme the feedback names.
...
- State the target across compact, medium and expanded widths, light, dark and outdoor themes, and 200
  percent text, whenever the feedback touches layout (FE-RESP-10, FE-A11Y-03).

## 6. When to add a human-review stop
